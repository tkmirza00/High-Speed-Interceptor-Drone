...
def make_pointcloud2(points, frame_id: str, stamp) -> PointCloud2:
    """Convert RadarPoint list to PointCloud2.

    Fields: x, y, z, doppler, snr, noise. Existing consumers that only read
    x/y/z/doppler/snr still work because offsets are unchanged.
    """
    msg = PointCloud2()
    ...
    data = bytearray()
    for p in points:
        noise_db = getattr(p, 'noise_db', 0.0)
        data += struct.pack('<ffffff', p.x, p.y, p.z, p.doppler, p.snr_db, noise_db)
    msg.data = bytes(data)
    return msg
